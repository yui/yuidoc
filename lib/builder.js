...
YUI.add('doc-builder', function(Y) {
    ...
    Y.DocBuilder = function(options, data) {
        ...
        Y.Handlebars.registerHelper('crossLinkModule', function(item, fn) {
            ...
            if (self.data.modules[item]) {
                str = '<a href="../modules/' + item + '.html">' + item + '</a>';
            }
            return str;
        });
    ...
    Y.DocBuilder.prototype = {
        ...
        _mixExternal: function() {
            ...
            var exData = self.options.externalData;
        ...
        */
        ...
        getProjectMeta: function() {
            var obj = {
                meta: {
                    yuiSeedUrl: 'http://yui.yahooapis.com/3.5.0pr4/build/yui/yui-min.js',
                    yuiGridsUrl: 'http://yui.yahooapis.com/3.5.0pr4/build/cssgrids/cssgrids-min.css'
                }
            };
            if (!this._meta) {
                try {
                    ...
                    if (fs.existsSync(path.join(themeDir, 'theme.json'))) {
                        meta = Y.Files.getJSON(path.join(themeDir, 'theme.json'));
                    } else if (DEFAULT_THEME !== themeDir) {
                        if (fs.existsSync(path.join(DEFAULT_THEME))) {
                            meta = Y.Files.getJSON(path.join(DEFAULT_THEME, 'theme.json'));
                        }
                    }
        ...
        */
        ...
        renderModule: function(cb, data, layout) {
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                opts.meta.moduleDescription = self._parseCode(markdown(data.description || ' '));
        ...
        */
        ...
        renderClass: function(cb, data, layout) {
            ...
            Y.prepare([DEFAULT_THEME, themeDir], self.getProjectMeta(), function(err, opts) {
                ...
                opts.meta.classDescription = self._parseCode(markdown(data.description || ' '));
        ...
        */
        ...
        writeAPIMeta: function(cb) {
            ...
            this.renderAPIMeta(function(js) {
                fs.writeFile(path.join(self.options.outdir, 'api.js'), js, 'utf8', cb);
            });
        },
