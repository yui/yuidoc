    ...
    DocParser.ATTRS = {
        ...
        */
        mainmodule: {
            setter: function(o) {
                var write = true;
                if (this.get(CURRENT_MODULE) === o.name) {
                    if (o.name in this.modules) {
                        if (this.modules[o.name].tag) {
                            //The main module has already been added, don't over write it.
                            if (this.modules[o.name].tag === 'main') {
                                write = false;
                            }
                        }
                        if (write) {
                            this.modules[o.name] = Y.merge(this.modules[o.name], o);
                        }
                    }
                }
            }
