...
YUI.add('utils', function (Y) {
    ...
    **/
    ...
    var validatePaths = function(paths, ignore) {
				...
				return [].concat(paths || process.cwd())
					.reduce(function(paths, path){ return paths.concat(glob.sync(path)); }, [])
					.filter(function(path){
							return !ignore.filter(function(ignoreGlob){ return minimatch(path, ignoreGlob); }).length;
					})
					.filter(function(path, i, paths){ 
							return !paths.slice(i+1).filter(function(path2){ return path === path2; }).length; 
					});
    };
